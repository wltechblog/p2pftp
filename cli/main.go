...
a=sctpmap:5000 webrtc-datachannel 1024`
			...
			c.ui.LogDebug("Offer sent")

		case "answer":
			c.ui.LogDebug("Received answer")
			if err := c.SendMessage(Message{
				Type:      "ice",
		...
		case "ice":
			c.ui.LogDebug("Received ICE candidate")
			c.webrtc.connected = true
			c.ui.ShowConnectionAccepted("ICE received - connection established")
...
func (c *Client) SendMessage(msg Message) error {
	c.ui.LogDebug(fmt.Sprintf("→ Sending %s to %s", msg.Type, msg.PeerToken))
