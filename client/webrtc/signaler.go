...
func (s *Signaler) handleMessages() {
	for {
		...
		case "request":
			// Don't set peerToken yet, just store the request token
			requestToken := msg.Token
			s.debugLog.Printf("Connection request from: %s", requestToken)

			// Notify about the request
			if s.peer != nil && s.peer.statusHandler != nil {
				s.peer.statusHandler(fmt.Sprintf("Connection request from: %s", requestToken))
			}

			// Store the request token for later use
			if s.peer != nil {
				s.peer.StoreRequestToken(requestToken)
			}
